...
def check_vehicle(vehicle, trip, calendar, calendar_dates, state):
    """runs all six field checks for one live vehicle against its
    schedule. also does the matching + state updates needed along the
    way (this is the one place that actually mutates state - callers
    should call state.save() once after processing everyone, not per
    vehicle, so a crash mid-cycle cant leave things half written)."""
    ...
    results = [
        check_arrival_timing(vehicle, matched_stop),
        check_departure_timing(vehicle, matched_stop),
...
def check_arrival_timing(vehicle, matched_stop):
    """field 1 - does the observed arrival at the matched stop line up
    with the scheduled arrival."""
...
def check_departure_timing(vehicle, matched_stop):
    """field 2 - same idea against departure_time. worth repeating the
    caveat: this uses the SAME observed timestamp as field 1 (one gps
    ping, not separate arrival/departure events like a real TripUpdate
    would give) just compared to a different scheduled time. can still
    genuinely disagree independently of field 1 if the stop has a
    scheduled dwell (arrival_time != departure_time)."""
    if matched_stop is None:
...
def check_trip_level_adherence(vehicle, calendar, calendar_dates, service_id):
    ...
    catches the "unscheduled extra" direction per-cycle (a live trip with
...
def check_direction(vehicle, trip, last_position):
    ...
    between this position and the vehicles last known one, and compare it
    against the bearing implied by the trip's own scheduled stops around
    here. a big mismatch means the vehicle is actually moving a way that
    doesn't match the trip its assigned to."""
...
def check_start_time_date(vehicle, trip, calendar, calendar_dates, service_id):
    ...
    real finding while building this: every single active vehicle on the
    route right now shows an exact -3600s (1 hour) drift here. checked
    it wasnt our own bug - the two epoch conversions go through the exact
    same function, so a bug in our own tz handling would cancel out, not
    produce a clean 3600s gap. the raw strings themselves are 1h apart
    (live says "18:41:00", schedule says "19:41:00"), and 18:41 + 1h =
    19:41 exactly - consistent with bods's live feed not applying the
    BST offset when it serializes start_time. real upstream data quality
    issue, not us. this is genuinely what field 6 is supposed to catch."""
    if not trip["stops"]:
...
def _expected_bearing_near(trip, lat, lon):
    ...
    position and returns the bearing between them - a rough stand-in for
    "which way is the route pointing right here" since we dont have
