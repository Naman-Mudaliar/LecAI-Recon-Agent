...
# anchor on our OWN memory. every trip we're tracking remembers the
# highest stop_sequence it's confidently matched so far (persisted across
# cycles in the ledger), and each new cycle we only look at stops AHEAD of
# that. pick whichever of those candidate stops is closest to the live
# position, and if its within the match radius, call it matched and move
# the anchor forward.
#
# this is what stops a route that loops back near itself (263 does
# exactly this around altrincham interchange) from matching the vehicle
# to the wrong pass of the loop - we're never doing a blind nearest-stop
# search across the whole route, only the next few stops after wherever
# we last confirmed the bus actually was.
